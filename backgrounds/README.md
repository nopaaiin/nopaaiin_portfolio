...
사진 위치는 `portfolio.css`의 `.hero-mosaic`에 있는 `background-position`으로 조정합니다. 격자 크기는 `mask-size`, 이동 속도는 `.hero-wash`의 애니메이션 시간으로 조정합니다. 운영체제에서 동작 줄이기를 사용하면 움직임이 멈춥니다.
